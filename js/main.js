...
// Function to load trending groups
async function loadTrendingGroups(period = 'today') {
    const trendingGrid = document.querySelector('.trending-groups-grid');
    ...
    try {
        ...
        trendingGrid.innerHTML = groups.map(group => createGroupCard(group)).join('');
        setupLazyLoading();
        setupViewTracking();
    } catch (error) {
...
// Function to load recent groups
async function loadRecentGroups() {
    const recentGrid = document.querySelector('.recent-groups-grid');
    ...
    try {
        ...
        recentGrid.innerHTML = groups.map(group => createGroupCard(group)).join('');
        setupLazyLoading();
        setupViewTracking();
    } catch (error) {
...
// Function to load popular groups by region
async function loadPopularRegionGroups(region = 'all') {
    const regionGrid = document.querySelector('.popular-region-grid');
    ...
    try {
        ...
        if (region === 'all') {
            ...
            const regionCountries = {
                ...
                'asia': ['India', 'Pakistan', 'Bangladesh', 'Singapore', 'Malaysia'],
                'africa': ['South Africa', 'Nigeria', 'Egypt', 'Kenya']
            };

            q = query(
                groupsRef,
                where("country", "in", regionCountries[region]),
                orderBy("views", "desc"),
                limit(6)
            );
        }
        ...
        regionGrid.innerHTML = groups.map(group => createGroupCard(group)).join('');
        setupLazyLoading();
        setupViewTracking();
    } catch (error) {
        console.error('Error loading popular region groups:', error);
    }
}

// Function to find similar groups
async function findSimilarGroups(groupId) {
    try {
        const groupRef = doc(db, "groups", groupId);
        const groupDoc = await getDoc(groupRef);
        ...
        // Query groups with same category and country
        const q = query(
            groupsRef,
            where("category", "==", groupData.category),
            where("country", "==", groupData.country),
            limit(5)
        );
        ...
        querySnapshot.forEach((doc) => {
            if (doc.id !== groupId) {
                similarGroups.push({ id: doc.id, ...doc.data() });
            }
        });
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    loadTrendingGroups();
    loadRecentGroups();
    loadPopularRegionGroups();

    // Trending filters
    const trendingFilters = document.querySelectorAll('.trending-filter');
    trendingFilters.forEach(filter => {
        filter.addEventListener('click', () => {
            trendingFilters.forEach(f => f.classList.remove('active'));
            filter.classList.add('active');
            loadTrendingGroups(filter.dataset.period);
        });
    });
    ...
    regionTabs.forEach(tab => {
        tab.addEventListener('click', () => {
            ...
            loadPopularRegionGroups(tab.dataset.region);
        });
    });
});
